...
$(document).ready(function () {
	...
	$('[bg-e-filltext]').on('click.filltext', function () {
		$('#' + $(this).attr('bg-e-filltext')).focus().val($(this).text()).addClass('mdl-textfield__input is-dirty').parents('.mdl-textfield').addClass('is-dirty');
	});

	$('[bg-e-go]').on('click.go', function () {
		window.location.assign($(this).attr('bg-e-go'));
	});

	$('[bg-e-input-array]').each(function () {
		var name = $(this).attr('bg-name');
		var $container = $('#' + $(this).attr('bg-e-input-array'));

		$(this).find(':checkbox').on('change.array', function () {
			var value = $(this).attr('bg-value');

			if (this.checked) {
				$('<input type="hidden"/>').attr('name', name + '[]').attr('value', value).appendTo($container);
			} else {
	...
	$('[bg-e-disable-related]').on('change.disable', function () {
		var checked = this.checked;
		$(this).attr('bg-e-disable-related').split('|').forEach(function (id) {
